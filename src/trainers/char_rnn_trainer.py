...
class CharRNNTrainer(BaseTrainer):
    ...
    def init_dataloaders(self):
        ...
        # vocab = pickle.load('data/char-rnn-finetuning/vocab-73060')
        field.vocab = self.read_pickle_module('vocab', self.config.firelab.continue_from_iter)
        ...
        self.val_dataloader = data.BucketIterator(
            self.val_ds, self.config.display_k_val_examples, shuffle=False, repeat=True)
    ...
    def loss_on_batch(self, batch):
        ...
        z = cudable(torch.zeros(self.config.hp.n_layers, batch.batch_size, self.config.hp.model_size))
    ...
    def validate(self):
        ...
        z = cudable(torch.zeros(self.config.hp.n_layers, len(batch.text), self.config.hp.model_size))
        z = self.lm(z, cudable(batch.text), return_z=True)[1]
        ...
        sources = itos_many(batch.text, self.vocab, sep='')

        texts = ['`{} => {}`'.format(s,r) for s,r in zip(sources, results)]
        text = '\n\n'.join(texts)
